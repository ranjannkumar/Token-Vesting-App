...
export type IncrementInstruction<
  ...
  TAccountTokenvesting extends string | AccountMeta<string> = string,
  TRemainingAccounts extends readonly AccountMeta<string>[] = [],
> = Instruction<TProgram> &
  InstructionWithData<ReadonlyUint8Array> &
  InstructionWithAccounts<
    [
      TAccountTokenvesting extends string
        ? WritableAccount<TAccountTokenvesting>
        : TAccountTokenvesting,
      ...TRemainingAccounts,
    ]
  >;
...
export type IncrementInput<TAccountTokenvesting extends string = string> = {
  tokenvesting: Address<TAccountTokenvesting>;
};
...
>(
  input: IncrementInput<TAccountTokenvesting>,
  config?: { programAddress?: TProgramAddress }
): IncrementInstruction<TProgramAddress, TAccountTokenvesting> {
  // Program address.
  const programAddress = config?.programAddress ?? TOKENVESTING_PROGRAM_ADDRESS;

  // Original accounts.
  const originalAccounts = {
    tokenvesting: { value: input.tokenvesting ?? null, isWritable: true },
  };
  ...
  const instruction = {
    accounts: [getAccountMeta(accounts.tokenvesting)],
    programAddress,
    data: getIncrementInstructionDataEncoder().encode({}),
  } as IncrementInstruction<TProgramAddress, TAccountTokenvesting>;
